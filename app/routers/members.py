...
def _attach(
    db: Session, account_id: str, member_type: str, workspace_id: str
) -> MemberRegisterOut:
    """Attach an EXISTING agent/bot_app account as a new per-workspace
    membership (spec §4): mints a fresh key for this workspace, reusing
    the account's existing member_name (from any prior membership) so its
    handle deduplicates locally (e.g. @analyst / @analyst2, Decision 2's
    example). A nonexistent account, or one of the wrong type, is
    indistinguishable from not-found -- same "the wall" pattern as every
    other cross-entity reference in this codebase. Already a member here
    -> 409 (AlreadyAMemberError), same code create_member_account uses
    for the human doors.
    """
    account = db.get(Account, account_id)
    if account is None or account.account_type != member_type:
        raise NotFoundError(f"Account '{account_id}' not found")
    existing = (
        db.query(Member)
        .filter(Member.workspace_id == workspace_id, Member.account_id == account_id)
        .first()
    )
...
def get_my_profile(
    ...
    """The caller's own full profile, including workspace_id and account_id.
